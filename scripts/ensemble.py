...
def run(in_path, out_path, in_names, out_name, weights):
    ...
    for in_name, weight in zip(in_names, weights):
        with open(os.path.join(in_path, in_name, 'result.txt')) as file:
            current_paths = np.array(tuple(map(lambda x: x.split()[0], file.readlines())))
            if paths is None:
                paths = current_paths
            assert np.array_equal(paths, current_paths)
        probablility = np.load(os.path.join(in_path, in_name, 'probability.npy'))
